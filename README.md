...
---
◆ v0.2.1：修好了「状态查询」误报未配置的问题——之前它读不到你填的数据库路径，会显示成"没配"（其实不影响注入，只是那个查询在瞎说）
...
| v0.2.0 | 睡眠解析修复定稿 + 版本号规整（详见下节） | 历史 |
| **v0.2.1** | **修复「状态查询」误报未配置**（详见下节） | **当前** |

### v0.2.1 修复「状态查询」误报

- **现象**：工具箱设置里明明填了数据库路径，内置的「状态查询」却一直报未配置、路径为空。
- **原因**：取 Android context 时只认全局 `getAppContext`，在工具模块里取不到，于是静默回落到「未配置」分支。
- **修法**：改为优先 `Java.getApplicationContext()`，取不到再兜底 `getAppContext()`，与注入链路取 context 的方式一致。
- **影响范围**：只影响这一个查询的显示，**注入本身一直正常**；不升级也不影响手环数据。
